...
<details id="custom-field"
    ...
    <ul class="fields mono">
        ...
        <?php foreach ($fields as $index => $field): ?>
            <?php $fieldId = 'custom-field-' . (int) $index; ?>
            <li class="field">
                <div class="field-name">
                    <label for="<?php echo $fieldId; ?>-name" class="sr-only"><?php _e('字段名称'); ?></label>
                    <input type="text" name="fieldNames[]" value="<?php echo htmlspecialchars($field['name']); ?>"
                           id="<?php echo $fieldId; ?>-name" pattern="^[_a-zA-Z][_a-zA-Z0-9]*$" oninput="this.reportValidity()" class="text-s w-100">
                    <label for="<?php echo $fieldId; ?>-type" class="sr-only"><?php _e('字段类型'); ?></label>
                    <select name="fieldTypes[]" id="<?php echo $fieldId; ?>-type">
                        <option
                ...
                <div class="field-value">
                    <label for="<?php echo $fieldId; ?>-value" class="sr-only"><?php _e('字段值'); ?></label>
                    <textarea name="fieldValues[]" id="<?php echo $fieldId; ?>-value" class="text-s w-100"
        ...
        <?php if (empty($defaultFields) && empty($fields)): ?>
            <?php $fieldId = 'custom-field-0'; ?>
            <li class="field">
                <div class="field-name">
                    <label for="<?php echo $fieldId; ?>-name" class="sr-only"><?php _e('字段名称'); ?></label>
                    <input type="text" name="fieldNames[]" placeholder="<?php _e('字段名称'); ?>" id="<?php echo $fieldId; ?>-name"
                ...
                <div class="field-value">
                    <label for="<?php echo $fieldId; ?>-value" class="sr-only"><?php _e('字段值'); ?></label>
                    <textarea name="fieldValues[]" placeholder="<?php _e('字段值'); ?>" id="<?php echo $fieldId; ?>-value"
