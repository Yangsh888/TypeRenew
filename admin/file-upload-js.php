...
$(document).ready(function() {
    ...
    Typecho.uploadFile = (function () {
        ...
        Typecho.getUploadPending = function () {
            return pending;
        };
        ...
        const enqueue = function (file) {
            ...
            if (!match || !Array.isArray(types) || !types.includes(match[1].toLowerCase())) {
            ...
            if (confirm('<?php _e('确认要删除文件 %s 吗?'); ?>'.replace('%s', file))) {
                ...
                $.post('<?php $security->index('/action/contents-attachment-edit'); ?>',
                    ...
                    function () {
                        ...
                        $(document).trigger('attachmentDeleted', [cid]);
                        $el.fadeOut(function () {
                            $(this).remove();
                            updateAttachmentNumber();
                        });
