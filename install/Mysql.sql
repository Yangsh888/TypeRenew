...
CREATE TABLE `typecho_mail_queue` (
  ...
  `payload` longtext,
  PRIMARY KEY (`id`), KEY `idx_status_sendat` (`status`, `sendAt`),
  KEY `idx_status_updated` (`status`, `updated`), KEY `idx_locked` (`lockedUntil`),
  UNIQUE KEY `uniq_dedupe` (`dedupeKey`)
) ENGINE=%engine% DEFAULT CHARSET=%charset% COLLATE=%collate%;

CREATE TABLE `typecho_mail_unsub` (
  `id` bigint unsigned NOT NULL auto_increment,
  `email` varchar(255) NOT NULL, `scope` varchar(32) NOT NULL,
  `created` int unsigned NOT NULL default 0,
  PRIMARY KEY (`id`), UNIQUE KEY `uniq_email_scope` (`email`, `scope`)
) ENGINE=%engine% DEFAULT CHARSET=%charset% COLLATE=%collate%;

CREATE TABLE `typecho_password_resets` (
  `id` bigint unsigned NOT NULL auto_increment,
  `email` varchar(150) NOT NULL, `token` varchar(64) NOT NULL,
  `created` int unsigned NOT NULL default 0, `expires` int unsigned NOT NULL default 0,
